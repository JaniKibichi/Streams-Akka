...
object TransformingStreamsApp extends App{
  implicit val actorSystem = ActorSystem("TransformingStream")
  implicit val actorMaterializer = ActorMaterializer()

  val MaxGroups = 100

  val path = Paths.get("src/main/resources/gzipped-file.txt.gz")

  val stream = FileIO.fromPath(path).via(Compression.gunzip())
